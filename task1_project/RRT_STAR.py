...
class RRT_Star():
    ...
    def plan(self, vision, start_x, start_y, goal_x, goal_y):
        ...
        road_map, path_x, path_y = self.GetPath(start_node, goal_node, node_x, node_y)
        return node_x, node_y, road_map, path_x, path_y
    ...
    def GetPath(self, start_node, goal_node, node_x, node_y):
      node_list = list(zip(node_x, node_y))
      road_map = self.GetRoadMap(start_node, node_list)
      path_x = [goal_node.x]
      path_y = [goal_node.y]
      parent = goal_node.parent_node
      while parent != None:
        path_x.append(parent.x)
        path_y.append(parent.y)
        parent = parent.parent_node
      return road_map, path_x, path_y
